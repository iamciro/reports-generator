...
class PDF(FPDF):
	...
	def service_info_section(self, service_order_number, service_datetime):
		...
		self.multi_cell(150,10,pdf['service_order_number'] + service_order_number,0,0)
		...
		self.multi_cell(130,10,pdf['service_datetime'] + service_datetime,0,0)	 
	...
	def client_info_section(self, client_name, client_phone_number):
		...
		self.multi_cell(140,10,pdf['client_name'] + client_name,1,1)
		...
		self.multi_cell(80,10,pdf['client_phone_number'] + client_phone_number,1,0)
	...
	def device_info_section(self, device_type, device_company, device_model):
		...
		self.multi_cell(100,10,pdf['device_type'] + device_type,1,1)
		...
		self.multi_cell(60,10,pdf['device_company'] + device_company,1,0)

		# Move to computed self.offset
		self.x = 200

		self.multi_cell(60,10,pdf['device_model'] + device_model,1,0)

	def problem_info_section(self, reported_problem):
		# Set X position
		self.x = 5

		self.cell(0, 45, pdf['reported_problem'], 0, 1, 'C')
	...
	def date_client_firm_section(self):
		...
		self.cell(0, 0, pdf['repair_date'], 0, 1)

		# Set X position
		self.x = 185

		self.cell(0, 0, pdf['client_firm'], 0, 1)
